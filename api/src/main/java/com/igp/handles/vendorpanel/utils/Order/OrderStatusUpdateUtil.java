...
public class OrderStatusUpdateUtil {
    ...
    public static Boolean   markOutForDelivery(int orderId, String fkAssociateId, String status, String orderProductIds){
        ...
        try{
            ...
            connection = Database.INSTANCE.getReadWriteConnection();
            statement = "update orders_products as op inner join trackorders as tk on " +
                "op.orders_products_id=tk.orders_products_id and  op.orders_id=tk.orders_id " +
                " set tk.outForDeliveryDate=now() ,op.orders_product_status= ?,op.dispatch_date=?, op.delivery_status = ? " +
                "where  op.orders_id=? and op.fk_associate_id in(?) and op.orders_products_id in" +
                " ( "+orderProductIds+") ";
            preparedStatement = connection.prepareStatement(statement);
            preparedStatement.setString(1,status);
            ...
            preparedStatement.setString(5,fkAssociateId);
            //     preparedStatement.setString(4,orderProductIds);
            logger.debug("sql query "+preparedStatement);
            Integer rowsUpdated = preparedStatement.executeUpdate();
            if (rowsUpdated>0){
    ...
    public static Boolean markDelivered(int orderId, String fkAssociateId, String status, String orderProductIds,int deliveryAttemptFlag){
        ...
        try{
            ...
            statement = "update orders_products as op inner join trackorders as tk on " +
                "op.orders_products_id=tk.orders_products_id and  op.orders_id=tk.orders_id  " +
                "set tk.deliveryDate = now(),tk.deliveredDate=now() , op.orders_product_status = ? ,op.delivery_status= 1," +
                "op.delivery_attempt = case when op.delivery_attempt = 2 then 3 else  "+ deliveryAttemptFlag +"" +
                " end where  op.orders_id=? and op.fk_associate_id in(?) and op.orders_products_id in " +
                "( "+orderProductIds+") ";
            preparedStatement = connection.prepareStatement(statement);
